...
void populateArray(FitbitData fitbit[], FILE *infile)
{
	...
	{ // loops until end of file
		...
		{ // tests to verrify target user & copies all data to fitbit data struct
...
{ //takes an array and a location and inserts a "-1" at the specified location
	...
	while (length - i >= location)
	{ // moves characters over until reaching designated insertion point
...
{ // loops through the fitbit array and adds up the total calories
	double Calories = 0.0;
	int i = 0;
	while (i <= 1440)
	{
...
{ // loops through the fitbit array and adds up the total distance
...
{ //calculates total flights climbed
	int Floors = 0;
	int i = 0;
	while (i <= 1440)
	{
...
{ // calculates avg heart over 24 hours with valid heart rates
	double heartTotal = 0;
	int i = 0;
	int count = 0;
	while (i <= 1440)
...
{ //calcualtes the most steps in one minute
	int maxsteps = 0;
	int i = 0;
	while (i <= 1440)
	{
...
{						  //loops through the sleep files to find the worst total time
	...
	for (i = 0; i <= 1440; i++)
	{
		...
		{ // finds point of wake up
			strcpy(tmp1.minuteStart, fitbit[i].minute);
			while (fitbit[i].sleepLevel > 1)
			{ // runns only while awake
				tmp1.totalTime += fitbit[i].sleepLevel;
				i++;
			}
			strcpy(tmp1.minuteEnd, fitbit[i - 1].minute);
		}
		if (tmp1.totalTime > worst.totalTime)
		{ //compares current to real worst time
